...
- Disabled entities are not polled: disable a meter's sensor under Settings -> Entities and that meter stops requesting the underlying register too - less Modbus traffic on the shared bus, not just a hidden entity. This takes effect live (no restart needed), and re-enabling an entity later brings it back into the next poll.
...
`coordinator.py` builds those read blocks not from the full register map but only from the registers whose sensor entity is currently enabled (via the entity registry, with an `entity_registry_enabled_default` fallback while an entity doesn't exist yet). A listener on entity registry changes (set up after the sensors are created in `__init__.py`) recomputes those blocks the moment you enable or disable an entity and immediately requests a fresh poll - so this works live, no restart required. `sensor.py` still creates an entity for every register (including disabled ones), so a previously disabled entity can simply be turned back on without re-adding the integration.
